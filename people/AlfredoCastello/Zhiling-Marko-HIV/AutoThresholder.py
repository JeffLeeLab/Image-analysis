# -*- coding: utf-8 -*-
"""
ImageJ / Fiji Auto_Threshold methods, vendored into this project.

Source
------
https://github.com/lamdao/vipy/blob/master/AutoThresholder.py (GPL-3.0)
Python port by Lam H. Dao of G. Landini's Fiji Auto_Threshold plugin,
itself based on ij.process.AutoThresholder.

Local changes (upstream is Python 2 and does not import under Python 3)
----------------------------------------------------------------------
* RenyiEntropy: `opt_threshold = ...` spanned three lines without a line
  continuation (a SyntaxError even under Python 2); wrapped in parentheses.
* Removed the `__main__` test block (Python 2 `print` statements, and it
  imports `MIVFile`, which is not part of this project).
* `np.finfo(np.float)` / `astype(np.float)` -> `float` (np.float was removed
  in NumPy 1.24).
* `np.ndarray(256)` -> `np.zeros(256)`: these hold cumulative histograms and
  are read before being fully written, and Java's `double[]` is zeroed.
* IJIsoData degenerate case returned `len(data) / 2`, a float under Python 3;
  now `//`, so the return value stays an integer bin index.
* Moments: `for di in np.arange(0, 256.)` used a float to index the histogram,
  an IndexError on any current NumPy; iterates ints now.
* Yen: P2_sq was built as an inclusive suffix sum of hnorm**2 minus the grand
  total, so every entry was <= 0 and the log(P1_sq*P2_sq) term never fired,
  giving thresholds unrelated to the method. Now the exclusive suffix sum the
  Java original computes.
* Intermodes smoothed an integer histogram in place, truncating to int on
  every pass; ImageJ smooths a `double[]`, so the working copy is now float.

Usage
-----
    hist = np.bincount(image_8bit.ravel(), minlength=256)
    t = AutoThresholder.Otsu(hist)        # or Execute(Methods.Otsu, hist)
    mask = image_8bit > t                 # Fiji's "dark background" convention

Every method takes a 256-bin histogram of an 8-bit image and returns a
threshold as a bin index (-1, or 0 from Execute(), signals failure).
"""
#------------------------------------------------------------------------------
import numpy as np
#------------------------------------------------------------------------------
...
def Intermodes(data):
    ...
    hist = data[minbin:maxbin+1].astype(float)
...
def MaxEntropy(data):
    ...
    threshold = -1
    max_ent = np.finfo(float).min # max entropy
...
def Yen(data):
    ...
    P1 = hnorm.cumsum()
    hnorm_sq = hnorm ** 2
    P1_sq = hnorm_sq.cumsum()
    P2_sq = np.zeros(256)
    P2_sq[:-1] = hnorm_sq[::-1].cumsum()[::-1][1:]
#    P2_sq = np.zeros(256)
#    P2_sq[255] = 0.0;
